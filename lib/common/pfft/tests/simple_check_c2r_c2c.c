  ...
  for(ptrdiff_t k0=local_N_start[0]; k0<local_N_start[0]+local_N[0]; k0++)
    for(ptrdiff_t k1=local_N_start[1]; k1<local_N_start[1]+local_N[1]; k1++)
      for(ptrdiff_t k2=local_N_start[2]; k2<local_N_start[2]+local_N[2]; k2++, m++) {
        if (k2 == 0)
          data[m] = DATA_INIT(k0+k1)+abs(k2-N[2]/2);
        else if ((N[2]%2 == 1) && (k2 <= N[2]/2))
          data[m] = DATA_INIT(k0+k1)+abs(k2-N[2]/2-1) + I*DATA_INIT(k0+k1)*(k2-N[2]/2-1);
        else
          data[m] = DATA_INIT(k0+k1)+abs(k2-N[2]/2) + I*DATA_INIT(k0+k1)*(k2-N[2]/2);
      }
}
